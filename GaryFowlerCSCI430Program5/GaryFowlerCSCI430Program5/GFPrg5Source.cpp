...
{
  //loop through algorithm until finished boolean is true
  while (!processTable.process->finished)
  {
    //determine what process starts first

    //if there is no other process waiting, then run the process

    //if there is another process, check to make sure the process doesnt have a shorter time

    //if the process has a shorter time, swap to that process instead

    //if all processes are done make finished set to true
